...
void          USBD_IntrEna (void) {
 ...
 // enable_irq(INT_USB0-16);  
   NVIC_EnableIRQ((IRQn_Type)USB0_IRQn); // wenxue
}


uint8_t USB_ClockInit(void) // wenxue need to modify
{
    /* IRC48M clock */
    SIM->SOPT2 |= SIM_SOPT2_USBSRC_MASK;   // wenxue
...
//        printf("USB  Init failed, clock must be 96M or 48M\r\n");
    ...
    // wenxue
   /* Enable the IRC48M for USB Module */
    USB0->CLK_RECOVER_IRC_EN |= USB_CLK_RECOVER_IRC_EN_IRC_EN_MASK;
    USB0->CLK_RECOVER_CTRL = USB_CLK_RECOVER_CTRL_CLOCK_RECOVER_EN_MASK|USB_CLK_RECOVER_CTRL_RESTART_IFRTRIM_EN_MASK;
...
void USBD_Reset (void) {
  ...
  for (i = 1; i < 16; i++) {
    USB0->ENDPOINT[i].ENDPT = 0x00;            // wenxue 
  }
...
void USBD_WakeUp (void) {
  ...
  if (USBD_DeviceStatus & USB_GETSTATUS_REMOTE_WAKEUP) {
    USB0->CTL |=  USB_CTL_RESUME_MASK;  // wenxue !!
    while (i--)   asm("nop");
    USB0->CTL &= ~USB_CTL_RESUME_MASK;
  }
}

void USBD_WakeUp1 (void) {
  uint32_t i = 50000;
  {
    USB0->CTL |=  USB_CTL_RESUME_MASK;  // wenxue !!
    while (i--)   asm("nop");
